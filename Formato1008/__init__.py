...
blob_name_to_save = 'Formato1008-'+ str(date.today())+'.xlsx' # Archivo excel a guardar en el blob correspondiente al formato
MupiosenBlob = "Municipios_de_Colombia.xlsx"
# Definicion de los conceptos para el formato tipo SIESA
conceptos = {"1315":[130500,131000,131500], 
             "1316":[],
             "1317":[132000,136000,138000,139000],
             "1318":[130590] # buscar deterioro
}
...
def WorkSiesa(container_name,balanceFile,blob_name_DB,idEjecucion,idProcedencia):#,rentaPath):  
    """
    Elabora y guarda el formato 1008 en el Blob Storage, almacena las comprobaciones en BD.
    Args:
...
def GetClientsByConcept(DatosSeparados, ColumnaValorIngreso):
    ...
    for clave in conceptos:
        ...
        for conc in conceptos[clave]:
            if conc==130500 and clave=="1315":
                data = UnificarClientesPorCuenta(DatosSeparados,conc,conc+90 ,"Ingresos Brutos Recibidos",ColumnaValorIngreso)
            else:
                data = UnificarClientesPorCuenta(DatosSeparados,conc,(conc+ (100 if str(conc).endswith("00") else 1)),"Ingresos Brutos Recibidos",ColumnaValorIngreso)
